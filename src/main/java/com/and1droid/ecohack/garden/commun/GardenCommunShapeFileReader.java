...
public class GardenCommunShapeFileReader extends ShapeFileReader<ShpPolygon> {

    protected ShapeFileItem createData(ShpPolygon shape) {
        ShapeFileItem shapeFileItem = new ShapeFileItem();
        shapeFileItem.setId(shape.getParams().get("Index").trim());
        shapeFileItem.setName(shape.getParams().get("Text_").trim());
        shapeFileItem.setDescription(shape.getParams().get("NomAr_DESC").trim());
        shapeFileItem.setPoints(shape.getPointsAs3DArray());
        return shapeFileItem;
    }

    protected String getFileName() {
        return "Jardins_communautaires";
    }

    protected String getFolder() {
        String curDir = System.getProperty("user.dir");
        String folder = "/ecohack/garden/";
